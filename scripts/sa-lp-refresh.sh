...
DEPENDENCY_ORG_FILE="$HOME/My Drive/notes/public/value-aligned-research-advisors.org"
DEPENDENCY_ORG_PATH="public/value-aligned-research-advisors.org"
...
# The VARA public script embeds SALP's model, so this refresh regenerates both
# scripts. Refuse to package an unrelated saved or unsaved VARA edit into the
# automated site commit.
...
python3 "$STAFFORINI_REPO/scripts/generate-copycat-scripts.py" --profile all
...
commit_if_changed "$STAFFORINI_REPO" \
  "sa-lp: refresh returns/chart/calculator/code" \

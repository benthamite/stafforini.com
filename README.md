...
│   └── works.json         # Lightweight work metadata for dev rendering
├── layouts/
...
The dev server uses `hugo.dev.toml` for the note-editing workflow: it mounts
notes, posts, and lightweight static assets only. Quotes and the full work-page
corpus are rendered by full builds/deploys, not by the default local preview.

## Content workflow

All content is authored in org-mode and exported to Hugo-compatible markdown via ox-hugo:

1. **Notes** are exported from org files in `~/My Drive/notes/`
2. **Quotes** are exported from org files in `~/My Drive/bibliographic-notes/`
...
| Script | Purpose |
|---|---|
| `export-notes.sh` | Full note export + lastmod injection + backlinks + citing-notes |
| `export-quotes.sh` | Full quote export + ID-slug map + non-diary quotes + work pages + tags |
...
Use the repo-local test wrapper instead of the global `pytest` command:
...
The wrapper avoids a stale Homebrew `pytest` shim that points at a removed
Python interpreter. The suite includes a Hugo dev-build smoke test that verifies
the homepage still renders both recent notes and recent quotes.

## Deployment

Deploys are manual — there is no CI/CD from git pushes. The deploy script exports content, rebuilds derived data, builds locally, and pushes to Netlify via CLI:

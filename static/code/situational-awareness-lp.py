...
def _run(command, args):
    ...
    if command == 'calculator':
        ...
        filing_label = latest.get("form", "13F-HR")
        if filing_label == "Schedule 13G":
            latest_holding = next(
                h for h in latest["holdings"]
                if h.get("source") == "Schedule 13G")
            base_label = latest.get("base_disclosure", "").replace("_", " ")
            filing_desc = (
                f"Latest disclosed portfolio: {base_label} 13F plus Schedule 13G "
                f"filed {filing_date}: {latest_holding['ticker']} "
                f"{latest_holding['shares']:,} shares ({latest_holding['percent']}%)")
        else:

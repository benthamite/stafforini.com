...
def _run(command, args):
    ...
    if command == 'comparison':
        ...
        ret_scaled_d = _ret_series_ais(vals_scaled, dates_scaled)
    ...
    if command == 'sensitivity':
        ...
        def _safe_combo(label, delta_target, min_days, max_days):
            """Run one combination and let data failures abort the full sweep."""
    ...
    if command == 'delay':
        delay_quarters = [
            f["quarter"] for f in filings
            if f.get("form", "").startswith("13F")
        ]

        first_qe = quarter_end_dates[delay_quarters[0]]
        last_fd = filing_dates[delay_quarters[-1]]
        ...
        n = len(delay_quarters)
        i_T = [_date_idx(quarter_end_dates[q]) for q in delay_quarters]
        i_F = [_date_idx(filing_dates[q]) for q in delay_quarters]
        ...
        if n < 2:
            ...
            for s1 in s_doms[0]:
                if analysis_start <= s1:
                    factor = (
                        hold_factor(holdings[delay_quarters[0]], analysis_start, s1)
                        * hold_factor(holdings[delay_quarters[1]], s1, i_F[-1]))
                else:
                    factor = hold_factor(
                        holdings[delay_quarters[1]], analysis_start, i_F[-1])
            ...
            for s2 in s_doms[1]:
                ...
                for s1 in s_doms[0]:
                    if analysis_start <= s1:
                        factor = (
                            hold_factor(holdings[delay_quarters[0]], analysis_start, s1)
                            * hold_factor(holdings[delay_quarters[1]], s1, s2))
                    else:
                        factor = hold_factor(
                            holdings[delay_quarters[1]], analysis_start, s2)
                    factors.append(factor)
            ...
            for k in range(2, n - 1):
                prev_s = s_doms[k - 1]
                curr_s = s_doms[k]
                F_k = np.array([[hold_factor(holdings[delay_quarters[k]], sa, sb)
                                 for sb in curr_s]
                                for sa in prev_s])
                msg = (msg @ F_k) / len(prev_s)

            # Right boundary.
            F_last = np.array([hold_factor(holdings[delay_quarters[-1]], s, i_F[-1])
                               for s in s_doms[-1]])
            E_V_fund = float((msg * F_last).sum() / len(s_doms[-1]))

        # Copycat: Q_0 on [F_0, F_1], Q_j on [F_j, F_{j+1}] for j = 1..n-2.
        V_copy = hold_factor(holdings[delay_quarters[0]], analysis_start, i_F[1])
        for j in range(1, n - 1):
            V_copy *= hold_factor(holdings[delay_quarters[j]], i_F[j], i_F[j + 1])

        copycat_shortfall = E_V_fund / V_copy - 1.0
        window_days = i_F[-1] - analysis_start

        print("COPYCAT DELAY COST (equity proxy, uniform single-switch model)")
        print("=" * 68)
        print(f"Window: {filing_dates[delay_quarters[0]]} to "
              f"{filing_dates[delay_quarters[-1]]} ({window_days} trading days)")
        print(f"Quarters modelled: {n - 1} transitions")
        print("-" * 68)
        print(f"E[V_fund] - 1                      {fmt(E_V_fund - 1):>10}")
        print(f"V_copy    - 1                      {fmt(V_copy - 1):>10}")
        print(f"Copycat shortfall vs fund          {fmt(copycat_shortfall):>10}")
        print()
        print("Shortfall = E[V_fund] / V_copy - 1")
        print("Positive shortfall = delay hurts the copycat")
        return
    if command == 'calculator':
        ...
        # Fetch current underlying prices for all rows in the latest disclosure.
        ...
        def _fallback_current_option_contract(ticker, option_type, ref_date):
            ...
            if not underlying or underlying <= 0:
                return None
            lo, hi = _contract_window(ref_date, EXPIRY_MIN_DAYS, EXPIRY_MAX_DAYS)
            relaxed_lo, relaxed_hi = _contract_window(ref_date, 180, 730)
            try:
                tk = yf.Ticker(ticker)
                expiries = [
            ...
            for expiry in expiries:
                ...
                T = max((exp_dt - datetime.strptime(ref_date, '%Y-%m-%d')).days / 365.25, 1e-6)
                for _, row in rows.iterrows():
                    strike = _safe_float(row.get('strike'))
        ...
        for h in latest["holdings"]:
            if h["type"] in ('call', 'put'):
                ...
                if key in opt_contracts:
                    continue
                selected_on = h.get("selected_on", latest_fd)
                cache = _load_option_cache(h["ticker"], h["type"])
                selected_rows = cache[(cache['selected_on'] == selected_on)
                ...
                if selected_rows.empty:
                    ...
                    try:
                        chain = _get_or_fetch_chain(
                            h["ticker"], selected_on, h["type"], _marketdata_key())
                        contract = _select_marketdata_contract(
                            chain, selected_on, h["type"],
                            delta_target=OPTION_DELTA,
                            min_days=EXPIRY_MIN_DAYS,
                            max_days=EXPIRY_MAX_DAYS)
                    except Exception:
                    ...
                    if contract is None:
                        contract = _fallback_current_option_contract(
                            h["ticker"], h["type"], selected_on)
                    if contract:
                        selected_rows = pd.DataFrame([{
                            'date': contract.get('date', selected_on),
        ...
        def build_mode_data(positions, option_proxy=False,
            ...
            rows = []
            for (ticker, pos_type), value in positions.items():
                # Issuer-disclosure layers value shares at a market price, so they
                # carry sub-cent fractions that 13F rows never have. Derive every
                # basis from the same cent-rounded reported value so they agree.
                value = round(value, 2)
                underlying_price = None
                row = {"ticker": ticker, "type": pos_type,
                       "reported_value": value}
                if (ticker, pos_type) in position_meta:
                    row.update(position_meta[(ticker, pos_type)])
        ...
        def _warrant_note(h):
            if not h.get("warrant_shares"):
                return ""
            return f", including {h['warrant_shares']:,} pre-funded warrants"


        if latest.get("base_disclosure"):
            layered = sorted(
                (h for h in latest["holdings"] if h.get("source")),
                key=lambda h: h["filing_date"])
            base_label = latest["base_disclosure"].replace("_", " ")
            updates = "; ".join(
                f"{h['source']} filed {h['filing_date']}: {h['ticker']} "
                f"{h['shares']:,} shares"
                + (f" ({h['percent']}%)" if h.get("percent") is not None else "")
                + _warrant_note(h)
                for h in layered)
            filing_desc = f"Latest disclosed portfolio: {base_label} 13F plus {updates}"
        else:
            adjusted = [h for h in latest["holdings"] if h.get("warrant_shares")]
            filing_desc = (
                f"Latest disclosed portfolio: {quarter} 13F filed {filing_date}"
                + "".join(
                    f"; {h['ticker']} adds {h['warrant_shares']:,} pre-funded "
                    f"warrants from the {h['warrant_source']}"
                    for h in adjusted))
        ...
        BODY = (
            ...
            '<div class="meta">\n'
            '  %s &middot; underlying prices as of %s\n'
            '</div>\n'
            '<div id="output"></div>\n'
        ) % (scaled_pct, filing_desc, today)
...
def _parser():
    parser = argparse.ArgumentParser(description='Situational Awareness LP copycat analysis')
    ...
    parser.add_argument('--cache-dir', type=Path, default=Path.home() / '.cache' / 'stafforini-copycat' / 'situational-awareness-lp',

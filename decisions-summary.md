...
| 023 | ox-blackfriday details overflow | Scoped advice in `export-common.el` answers the details-summary regexp with a literal search; `check-export-elisp.sh` guards it. Upstream PR prepared but declined. | Final |
| 024 | Note lastmod stamping | The `#+lastmod:` keyword stays authoritative; the stafforini package registers its stamping hook through autoload cookies so it is active from startup. Stale keywords are set to the edit time, not the repair time. | Final |
| 025 | Notes-tree traversal | No shell-loop traversals of `~/My Drive/notes` from an Emacs-parented agent shell: macOS 26.4 XProtect kills the process tree. Use repository entry points or single files. | Final |
